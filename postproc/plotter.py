...
# Functions
# ------------------------------------------------------ COUNTOURS
def animate_predictions(predictions, reals, time, lim_p, lim_r, cmap, lvls, field_name, fname, **kwargs):
    global cf1, cf2, cf
    ...
    def animate(i):
        ...
        cf1 = ax1.contourf(x, y, predictions[i].T, 50, vmin=lim_p[0], vmax=lim_p[1], norm=norm_p, cmap=cmap)
        cf2 = ax2.contourf(x, y, reals[i].T, 50, vmin=lim_r[0], vmax=lim_r[1], norm=norm_r, cmap=cmap)

        title = r'$t = ' + '{:.1f}'.format(time[i]) + '$'
        ax1.set_title(title, size=12, y=1.03)
        return [cf1, cf2]

    N, M = predictions[0].shape[0], predictions[0].shape[1]
    if not 'x' in kwargs:
        xmin, xmax = 0, N - 1
    else:
    ...
    field = kwargs.get('field', r'$\left\langle \sigma^\prime \gamma^\prime\right\rangle$')

    x, y = np.linspace(xmin / scaling, xmax / scaling, N), np.linspace(ymin / scaling, ymax / scaling, M)
    x, y = x + xshift, y + yshift
    x, y = np.meshgrid(x, y)

    norm_p = mpl_colors.Normalize(vmin=lim_p[0], vmax=lim_p[1])
    norm_r = mpl_colors.Normalize(vmin=lim_r[0], vmax=lim_r[1])

    fig, (ax1, ax2) = plt.subplots(2, 1)
    cf1 = ax1.contourf(x, y, predictions[0].T, 50, vmin=lim_p[0], vmax=lim_p[1], norm=norm_p, cmap=cmap)
    cf2 = ax2.contourf(x, y, reals[0].T, 50, vmin=lim_r[0], vmax=lim_r[1], norm=norm_r, cmap=cmap)
    ...
    grey_color = '#dedede'
    cyl1 = patches.Circle((0, 0), 0.5, linewidth=0.2, edgecolor='black', facecolor=grey_color)
    cyl2 = patches.Circle((0, 0), 0.5, linewidth=0.2, edgecolor='black', facecolor=grey_color)
    ax1.add_patch(cyl1)
    ax2.add_patch(cyl2)
    plt.subplots_adjust(hspace=0.05, bottom=0.15)
    ax1.text(10.5,-2.7, r'$\mathrm{CNN}$')
    ax2.text(10.5,-2.7, r'$\mathrm{real}$')
    # -- Add colorbar
    cbar_ax = plt.colorbar(cf2, ax=[ax1, ax2]).ax
    cbar_ax.set_title(field_name, y=1.02, loc='left', size=12)
    colorbar.ColorbarBase(cbar_ax, norm=norm_r, cmap=cmap)

    # Animate
    writer = animation.FFMpegWriter(fps=fps, extra_args=['-vcodec', 'libx264'])
    anim = animation.FuncAnimation(fig, animate, frames=len(predictions))
